...
function doPost(e) {
  try {
    ...
    // Verify shared secret (optional for now, can be enabled later)
    const expectedSecret = PropertiesService.getScriptProperties().getProperty('FORM_SECRET');
    if (expectedSecret) {
      const providedSecret = data.secret || (e.postData && e.postData.headers && e.postData.headers['X-SITE-SECRET']);
